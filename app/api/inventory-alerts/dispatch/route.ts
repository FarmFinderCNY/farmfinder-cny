...
function normalizeProduct(value: string) { return value.trim().toLowerCase().replace(/\s+/g, " "); }
...
export async function POST(request: Request) {
  ...
  if (!supabaseUrl || !publishableKey || !serviceRoleKey || !resendKey) return NextResponse.json({ error: "Alert delivery is not configured." }, { status: 503 });
  if (!authorization?.startsWith("Bearer ")) return NextResponse.json({ error: "Authentication required." }, { status: 401 });

  let payload: DispatchPayload; try { payload = await request.json() as DispatchPayload; } catch { return NextResponse.json({ error: "Invalid request." }, { status: 400 }); }
  ...
  if (!farmId || !inventoryItemId) return NextResponse.json({ error: "Farm and product are required." }, { status: 400 });
  ...
  if (!farm || farm.owner_user_id !== userData.user.id) return NextResponse.json({ error: "You do not manage this farm." }, { status: 403 });
  if (!item || (item.status !== "available" && item.status !== "low")) return NextResponse.json({ sent: 0 });

  const { data: subscriptions, error: subscriptionError } = await serviceClient.from("inventory_alert_subscriptions").select("id,email,product_name").eq("farm_id", farmId).eq("active", true);
  if (subscriptionError) return NextResponse.json({ error: "Unable to check alerts." }, { status: 500 });
  const itemName = normalizeProduct(item.name);
  const matches = ((subscriptions ?? []) as AlertSubscription[]).filter((subscription) => { const requestedProduct = normalizeProduct(subscription.product_name); return requestedProduct === "__farm_updates__" || (requestedProduct && (itemName.includes(requestedProduct) || requestedProduct.includes(itemName))); });
  ...
  for (const subscription of matches) {
    ...
    const emailResponse = await fetch("https://api.resend.com/emails", { method: "POST", headers: { Authorization: `Bearer ${resendKey}`, "Content-Type": "application/json" }, body: JSON.stringify({
    ...
    if (!emailResponse.ok) { console.error("Inventory alert email failed:", await emailResponse.text()); continue; }
