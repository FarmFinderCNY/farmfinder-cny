...
export async function GET(request: Request) {
  ...
  const [farms, inventory, subscriptions, submissions, claims, updates] = await Promise.all([
  ...
  checks.push(databaseError
    ? { key: "database", label: "Core database structure", status: "error", detail: databaseError.message }
    : { key: "database", label: "Core database structure", status: "healthy", detail: "Required owner, inventory, alert, submission, claim, and update fields are available." });

  checks.push(process.env.RESEND_API_KEY
    ? { key: "email", label: "Email delivery configuration", status: "healthy", detail: "The server email connection is configured." }
  ...
  for (const farm of activeFarms) {
    const key = [normalize(farm.name), normalize(farm.address), normalize(farm.city)].join("|");
    if (key !== "||") groups.set(key, (groups.get(key) ?? 0) + 1);
  }
  const duplicateGroups = Array.from(groups.values()).filter((count) => count > 1).length;
  checks.push(duplicateGroups
    ? { key: "duplicates", label: "Duplicate active listings", status: "warning", detail: `${duplicateGroups} possible duplicate group${duplicateGroups === 1 ? "" : "s"} found by name and address.` }
    : { key: "duplicates", label: "Duplicate active listings", status: "healthy", detail: "No duplicate active farms were found by name and address." });
