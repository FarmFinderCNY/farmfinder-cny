...
export async function POST(request: Request) {
  try {
    const { pickup, dropoff } = (await request.json()) as {
      pickup?: AddressInput;
      dropoff?: AddressInput;
    };

    if (!pickup?.street_address?.[0] || !pickup.city || !pickup.state || !pickup.zip_code ||
        !dropoff?.street_address?.[0] || !dropoff.city || !dropoff.state || !dropoff.zip_code) {
    ...
    if (!authResponse.ok || !auth.access_token) {
      console.error("Uber Direct authentication failed", auth);
      return NextResponse.json({ error: "Uber Direct authentication failed." }, { status: 502 });
    }

    const normalize = (address: AddressInput) => JSON.stringify({
    ...
    const quoteResponse = await fetch(`${API_URL}/${encodeURIComponent(customerId)}/delivery_quotes`, {
      ...
      body: JSON.stringify({
        pickup_address: normalize(pickup),
        dropoff_address: normalize(dropoff),
      }),
      cache: "no-store",
    });
    ...
    // This route only creates a quote. It never calls Uber's create-delivery endpoint.
