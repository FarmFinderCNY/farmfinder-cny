...
function getMatchingProducts(stand: FarmStand, search: string) {
  const query = normalizeSearchText(search);
  if (!query) return [];
  const queryTerms = expandedSearchTerms(query);

  return (stand.inventory ?? []).filter((product) => {
    const productName = normalizeSearchText(product.name);
    return queryTerms.some((term) => productName.includes(term)) ||
      matchesWithLightTypoTolerance(query, productName);
  });
}
  ...
  const filtered = useMemo(() => {
    ...
    return stands
      .map((stand, index) => {
        ...
        const matchingProducts = query ? getMatchingProducts(stand, search) : inventory;
      ...
      <div className="filter-row" aria-label="Filter by product category">
        {["All", "Seasonal", "Produce", "Growing practices", ...standardCategories].map((item) => (
          <button key={item} type="button" className={category === item ? "active" : ""} aria-pressed={category === item} onClick={() => updateCategory(item)}>
            {item === "Seasonal" ? "🌱 Seasonal" : item === "Produce" ? "🥕 Vegetables" : item === "Growing practices" ? "🌿 Growing practices" : item}
          </button>
        ))}
      </div>
