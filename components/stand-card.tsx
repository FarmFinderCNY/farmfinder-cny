...
export function StandCard({ stand, distanceMiles = null, matchingProducts = [], listedProductMatch = false, searchLabel = "" }: { stand: FarmStand; distanceMiles?: number | null; matchingProducts?: InventoryItem[]; listedProductMatch?: boolean; searchLabel?: string; }) {
  ...
  const directions = stand.latitude !== null && stand.longitude !== null ? `https://www.google.com/maps/dir/?api=1&destination=${stand.latitude},${stand.longitude}` : `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location)}`;
  const isMarket = stand.listing_type === "farmers_market"; const attendingVendors = (stand.market_vendors ?? []).filter((vendor) => vendor.is_attending);
  const availableToday = (stand.inventory ?? []).filter((item) => item.status === "available" || item.status === "low");
  const inventoryUpdatedAt = stand.farmer_inventory_updated_at ? new Date(stand.farmer_inventory_updated_at).getTime() : null; const now = Date.now();
  ...
  return <article className={`stand-card ${ownerManaged ? "owner-managed-card" : ""}`}>
    {stand.photo_url && <img className="stand-photo" src={stand.photo_url} alt={isMarket ? `${stand.name} farmers market` : `${stand.name} farm`} />}
    <div className="card-topline"><span className="status"><i /> Active listing</span><div className="verification-badges">{isMarket && <span className="verified">🧺 {getMarketLabel(stand.name)}</span>}{ownerManaged ? <span className="owner-managed">✓ Owner Managed</span> : stand.is_verified ? <span className="verified">✓ Listing verified</span> : null}</div></div>
    ...
    {ownerManaged && <div className="owner-managed-explainer"><strong>✓ Updated by this farm</strong><span>Availability can be managed directly by the farm owner or operator.</span></div>}
    {isMarket ? <div className="inventory-summary"><strong>Vendors at this market</strong><span>{attendingVendors.length > 0 ? attendingVendors.slice(0, 3).map((vendor) => vendor.vendor_name).join(" · ") + (attendingVendors.length > 3 ? ` · +${attendingVendors.length - 3} more` : "") : "Vendor list coming soon"}</span></div> : <>
      {practiceBadge && <span className="practice-card-badge">🌱 {practiceBadge}</span>}{stand.submission_type === "community" && !stand.owner_user_id && <p className="community-attribution">Community submitted{stand.submitted_by_display_name ? ` by ${stand.submitted_by_display_name}` : ""} · Not yet owner managed</p>}
    ...
    <div className="card-links"><Link href={`/farms/${stand.id}`} onClick={() => recordAnalyticsEvent("farm_detail_view", { farmId: stand.id })}>{isMarket ? "View market" : "View details"}</Link><a href={directions} target="_blank" rel="noreferrer" onClick={() => recordAnalyticsEvent("directions_click", { farmId: stand.id })}>Directions ↗</a></div>
  </article>;
}
